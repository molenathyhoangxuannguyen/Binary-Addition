...
public:
	...
	*/
	void increment() {
		Bit* bitRef = leastSignificantBit;
		//bitRef->add_next_bit(false);
		//bitRef = bitRef->get_next_bit();
		int remainder = 1;
		for (int i = 0; i < numBits-1; i++) {
			int byte = bitRef->get_bit();
			cout << "The bit is being implemented : " << byte << endl;
			if (byte == 0 && remainder == 0) {
				byte = 0;
				remainder = 0;
			}
			else if (byte == 1 && remainder == 0) {
				byte = 1;
				remainder = 0;
			}

			else if (byte == 0 && remainder == 1) {
				byte = 1;
				remainder = 0;
			}
			else {
		...
		int byte = bitRef->get_bit();
			cout << "The left-most bit is: " << byte << endl;
			if (remainder == 1 && byte == 1) {
				byte = 0;
				
				bitRef->set_bit(byte);
				bitRef->add_next_bit(true);
				numBits += 1;
			}
			else {
				byte = 1;
